...
var passwordCheckOld = function () 
{
    ...
    stringPassword = $("password").value; //dom input from users
    ...
    stringOutput = "";// this one is for users' passworld whatever is in the quotations
 /* insert code
   * here
   */

    if(stringPassword. length < 8)  //if statement checking if password is too short, less than 8 
    {
        stringOutput="Sorry,password length is too short."; // this one is to tell users what problem is here
        $("output").value = stringOutput;
    }

    else     //if the passworld is not too short, it should be check if it matches a bad password
    {
        for (  i=0; i<arrayBadPasswords.length; i++)
        {
            if (stringPassword===arrayBadPasswords[i])
            {
                stringOutput="Sorry,bad password. Please Try again.";
                $("output").value = stringOutput;
                break;
